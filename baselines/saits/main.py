"""
Multi-layer model training procedure for one split
We will train one split each time
"""
import numpy as np
import pandas as pd
...
def main(args, split_idx):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (input_feat, lapr_feat, step_gt, max_sr, sr_mean, sr_std, pid_ids) in enumerate(train_loader):
            ...
            print(f"iter {idx} time: {(time.time() - iter_start_time):.2f} seconds")
        ...
        if args.verbose:
            ...
            print(f"split: {split_idx} | epoch: {epoch} | train_mae: {train_avg_mae:.4f} | valid_mae: {valid_avg_mae:.4f} | valid_macro_mae: {valid_macro_mae:.4f}" \
                f"| test_mae: {test_avg_mae:.4f} | test_macro_mae: {test_macro_mae:.4f} | epoch_time: {(time.time() - epoch_start_time):.2f} seconds")
        ...
        if valid_avg_loss < best_val_nll:
            ...
            if args.save:
                model_file_name = f"{OUT_PATH}/best_model_self_attn_multilayer_lapr_split_{split_idx}_seed_{args.seed}_dk_{args.d_k}_kh_{args.kh}_kw_{args.kw}"
                if args.pid_feat:
    ...
    if args.save:
        loss_file_name = f"{OUT_PATH}/loss_history_self_attn_multilayer_lapr_split_{split_idx}_seed_{args.seed}_dk_{args.d_k}_kh_{args.kh}_kw_{args.kw}"
        if args.pid_feat:
            loss_file_name += "_with_pid"
        if not args.pad_full_weeks:
            loss_file_name += "_same_day"
        if args.lapr_rep:
            loss_file_name += "_lapr_rep"
        loss_file_name += ".pkl"
...
if __name__ == "__main__":
    ...
    new_dir(f"./results")
    OUT_PATH = f"./results/self_attn_multilayer_lapr_{args.d_k}_{args.d_v}_{args.kh}_{args.kw}"
    new_dir(OUT_PATH)
    ...
    plt.savefig(f"{OUT_PATH}/allofus_{timestamp}_self_attn_multilayer_lapr_{args.lr}_{args.batch_size}_{args.d_k}_{args.d_v}_{args.kh}_{args.kw}_{args.seed}_split_{args.split_idx}.png",\
                dpi=300)
    
    # copy the model from the VM disk to google cloud bucket
    if args.save:
        model_file_name = f"{OUT_PATH}/best_model_self_attn_multilayer_lapr_split_{args.split_idx}_seed_{args.seed}_dk_{args.d_k}_kh_{args.kh}_kw_{args.kw}"
        if args.pid_feat:
            model_file_name += "_with_pid"
        if not args.pad_full_weeks:
            model_file_name += "_same_day"
    ...
    if args.save:
        loss_file_name = f"{OUT_PATH}/loss_history_self_attn_multilayer_lapr_split_{args.split_idx}_seed_{args.seed}_dk_{args.d_k}_kh_{args.kh}_kw_{args.kw}"
        if args.pid_feat:
            loss_file_name += "_with_pid"
        if not args.pad_full_weeks:
            loss_file_name += "_same_day"
