"""
MRNN model with LAPR training procedure
"""
import numpy as np
import pandas as pd
...
from baselines.mrnn.dataset import AllOfUsDatasetLAPR, BatchCollate
...
def main(args, split_idx):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (input_feat, lapr_feat, step_gt, max_sr, sr_mean, sr_std, pid_ids) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            output = model(data) # forward pass shape: [bs, 1]  
        ...
        if args.verbose:
            ...
            print(f"split: {split_idx} | epoch: {epoch} | train_mae: {train_avg_mae:.4f} | valid_mae: {valid_avg_mae:.4f} | valid_macro_mae: {valid_macro_mae:.4f}" \
                  f"| test_mae: {test_avg_mae:.4f} | test_macro_mae: {test_macro_mae:.4f} | epoch_time: {epoch_time:.4f} seconds")
        ...
        if (valid_total_mae.item() / valid_total_num) < best_val_nll:
            ...
            if args.save:
                model_file_name = f"{OUT_PATH}/best_model_mrnn_lapr_split_{split_idx}_seed_{args.seed}_hidden_{args.rnn_hid_dim}"
    ...
    if args.save:
        loss_file_name = f"{OUT_PATH}/loss_history_mrnn_lapr_split_{split_idx}_seed_{args.seed}_hidden_{args.rnn_hid_dim}"
        if not args.pad_full_weeks:
            model_file_name += "_same_day"
        loss_file_name += ".pkl"
...
if __name__ == "__main__":
    ...
    new_dir(f"./results")
    OUT_PATH = f"./results/mrnn_{args.rnn_hid_dim}"
    new_dir(OUT_PATH)
    ...
    plt.savefig(f"{OUT_PATH}/allofus_{timestamp}_mrnn_{args.rnn_hid_dim}_{args.seed}_split_{args.split_idx}.png",\
                dpi=300)
    
    # copy the model from the VM disk to google cloud bucket
    if args.save:
        model_file_name = f"{OUT_PATH}/best_model_mrnn_lapr_split_{args.split_idx}_seed_{args.seed}_hidden_{args.rnn_hid_dim}"
        if not args.pad_full_weeks:
            model_file_name += "_same_day"
        model_file_name += ".pth"
