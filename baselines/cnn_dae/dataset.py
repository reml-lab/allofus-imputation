...
class AllOfUsDataset(Dataset):
    ...
    def process(self, pid_data, ks=(9, 15)):
        ...
        for part_id, (conv_feat, feature_list, step_rate_mean, step_rate_std) in enumerate(tqdm(pid_data)):
            ...
            input_feat = conv_feat_tensor[:, [feature_list.index("step_rate_norm"),
                                              feature_list.index("heart_rate_norm"),
                                              feature_list.index(f"{self.dataset}_mask_comp_split{self.split_idx}"), 
                                              feature_list.index(f"{self.dataset}_mask_split{self.split_idx}")
                                           ], :, :]
    ...
    def make_context_windows(self, x):
        ...
        # we need to transpose the kh and kw to make it column major representation
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2], self.ks[0], -1).transpose(-1, -2)
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2], -1) # [1, L, 6, kw*kh]
...
if __name__ == "__main__":
    ...
    pull_file("pid_data.pkl") # created in get_cohort_aaai.ipynb
    start_time = time.time()
    with open(f"{FILE_CACHE}/pid_data.pkl", "rb") as fin:
        pid_data = pickle.load(fin)
    print(f"load all conv_feat time: {(time.time()-start_time):.2f} seconds")

    # write every processed dataset into the google bucket
    #for split_idx in tqdm(range(10)):
    train_dataset = AllOfUsDataset(pid_data, split_idx=0, dataset="train", ks=ks, pad_full_weeks=pad_full_weeks)
    train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=False)
    
    for idx, (input_feat, step_gt, part_mean_sr, max_sr, sr_mean, sr_std, pid_ids) in enumerate(train_loader):
        if idx > 0:
            break
        print(input_feat.shape, step_gt.shape, part_mean_sr.shape, max_sr.shape, sr_mean.shape, sr_std.shape, pid_ids.shape)