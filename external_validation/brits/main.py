"""
This script is to evaluate BRITS on the external validation set 
(high and low miss rate)
"""
...
from external_validation.brits.dataset import AllOfUsDatasetExtValid
...
def main(args, split_idx):
    ...
    ### define the model ###
    model = brits.Model(rnn_hid_size=args.rnn_hid_dim, device=device, no_hr_loss=args.no_hr_loss)
    ...
    model_file_name = f"best_model_brits_split_{split_idx}_seed_{args.seed}_hidden_{args.rnn_hid_dim}"
    ...
    with torch.no_grad():
        ...
        for input_feat, step_gt, part_mean_sr, max_sr, sr_mean, sr_std, pid_ids in tqdm(test_loader):
            ...
            if args.all_gpus:
                ret_test = model.module.run_on_batch(data, None)
            else:
                ret_test = model.run_on_batch(data, None)
    ...
    if args.save_pred:
        ...
        with open(f"{OUT_PATH}/pred_results_dict_brits_split{split_idx}.pkl", "wb") as fout:
...
if __name__ == "__main__":
    ...
    new_dir(f"./results")
    OUT_PATH = f"./results/brits_pred_results_extvalid"
    if args.no_hr_loss:
        OUT_PATH += "_no_hr_loss"
    if args.if_high_miss:
        OUT_PATH += "_high_miss"
    else:
        OUT_PATH += "_low_miss"
