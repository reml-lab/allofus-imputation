"""
This script is to evaluate the model of CNN-DAE 
...
from external_validation.cnn_dae.dataset import AllOfUsDatasetExtValid
...
def main(args, split_idx):
    ...
    if args.if_high_miss:
        ...
        pull_file("df_cohort_extvalid_high_missrate.parquet")
        ...
        with open(f"{FILE_CACHE}/pid_data_extvalid_low_miss.pkl", "rb") as fin:
            pid_data = pickle.load(fin)
        # get the participant id list
        pull_file("df_cohort_extvalid_low_missrate.parquet")
    ...
    ## test ##
    test_dataset = AllOfUsDatasetExtValid(pid_data, ks=ks, pad_full_weeks=args.pad_full_weeks)
    ...
    model_file_name = f"best_model_cnn_dae_split_{split_idx}_seed_{args.seed}_kh_{args.kh}_kw_{args.kw}_nhr_{args.nhr_coeff}"
    ...
    with torch.no_grad():
        ...
        for input_feat, step_gt, part_mean_sr, max_sr, sr_mean, sr_std, pid_ids in tqdm(test_loader):
            ...
            output_sr_hr = model(dae_input_feat_corr, max_sr, sr_mean, sr_std) # forward pass shape: [bs, L, 2]  
            # we need to assess on the step count level
            output = output_sr_hr[:,0, output_sr_hr.shape[2]//2].unsqueeze(1) * sr_std + sr_mean  # step rate
            output = output.squeeze(1) * step_gt[:,1]  # step count

            test_mae = mae_loss(output, step_gt[:,0], mask=None, norm=True)
            test_mse = mse_loss(output, step_gt[:,0], mask=None, norm=True)
            ...
            if args.save_pred:
                ...
                pred_list.append(output.cpu().numpy()) # store the predicted step count
            ...
            for pid in range(len(pid_data)):
                ...
                test_pid_mae_list[pid].append(mae_loss(output, step_gt[:,0], mask=pid_mask.squeeze(1), norm=True).item()) # the mean 
                test_pid_mse_list[pid].append(mse_loss(output, step_gt[:,0], mask=pid_mask.squeeze(1), norm=True).item()) # the mean
                test_pid_num_list[pid].append(pid_mask.sum().item())
    ...
    if args.save_pred:
        ...
        with open(f"{OUT_PATH}/pred_results_dict_{args.model}_split{split_idx}.pkl", "wb") as fout:
...
if __name__ == "__main__":
    ...
    new_dir(f"./results")
    OUT_PATH = f"./results/cnn_dae_pred_results_extvalid"
    if args.if_high_miss:
        OUT_PATH += "_high_miss"
    else:
        OUT_PATH += "_low_miss"
    ...
    # pull the cnn_dae features
    if args.if_high_miss:
        feat_foldername = "cnn_feat_extvalid_high_miss/"
    else:
        feat_foldername = "cnn_feat_extvalid_low_miss/"
    if not os.path.exists(f"{FILE_CACHE}/{feat_foldername}"):
        os.system(f"gsutil -m cp -R {os.getenv('WORKSPACE_BUCKET')+'/data/'}{feat_foldername} {FILE_CACHE}")
