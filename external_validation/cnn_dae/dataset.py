...
class AllOfUsDatasetExtValid(Dataset):
    def __init__(self, pid_data, ks=(9, 15), pad_full_weeks=True):
        """
        Args:
    ...
    def process(self, pid_data, ks=(9, 15)):
        ...
        for part_id, (conv_feat, feature_list, step_rate_mean, step_rate_std) in enumerate(tqdm(pid_data)):
            ...
            step_gt = conv_feat_tensor[:, [feature_list.index("steps"), 
                                           ...
                                           feature_list.index(f"test_mask_split0")], :, :] # [1, 3, h, w]
            ...
            part_step_rate_mean = (test_steps * test_mask).sum() / (test_valid_min * test_mask).sum()
            ...
            input_feat = conv_feat_tensor[:, [feature_list.index("step_rate_norm"),
                                              ...
                                              feature_list.index("test_mask_split0")
                                           ], :, :]
