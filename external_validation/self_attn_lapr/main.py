...
from external_validation.self_attn_lapr.dataset import AllOfUsDatasetExtValid, BatchCollate
...
def main(args, split_idx):
    ...
    model = SelfAttnOneLayerLAPR(kernel_size=ks, 
                                 ...
                                 viz_mode=args.viz_attn)
    
    # load the trained model parameters
    model_file_name = f"best_model_self_attn_lapr_split_{split_idx}_seed_{args.seed}_dk_{args.d_k}_kh_9_kw_71"
    if args.pid_feat:
...
if __name__ == "__main__":
    ...
    new_dir(f"./results")
    OUT_PATH = f"./results/self_attn_lapr_{args.kh}_{args.kw}_pred_results_extvalid"
    if args.if_high_miss:
        OUT_PATH += "_high_miss"
    else:
        OUT_PATH += "_low_miss"
