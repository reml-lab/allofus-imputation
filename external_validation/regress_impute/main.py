"""
This script is to evaluate the linear regression model 
...
from external_validation.regress_impute.dataset import AllOfUsDatasetExtValid
...
def main(args, split_idx):
    ...
    model_file_name = f"best_model_regress_impute_split_{split_idx}_seed_{args.seed}_dk_{args.d_k}_kh_{args.kh}_kw_{args.kw}"
    ...
    if args.save_pred:
        ...
        with open(f"{OUT_PATH}/pred_results_dict_regress_impute_split{split_idx}.pkl", "wb") as fout:
...
if __name__ == "__main__":
    ...
    new_dir(f"./results")
    OUT_PATH = f"./results/regress_impute_pred_results_extvalid"
    if args.if_high_miss:
        OUT_PATH += "_high_miss"
    else:
        OUT_PATH += "_low_miss"
