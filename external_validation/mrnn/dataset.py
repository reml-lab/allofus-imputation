...
### Here since we only have one split for external validation, we set all the split_idx in the code as 0 ###
class AllOfUsDataExtValidMRNN(Dataset):
    def __init__(self, pid_list, pid_data, split_idx, dataset="test", ks=(9, 15), pad_full_weeks=True, if_high_miss=True):
        """
        Args:
            ...
            - if_high_miss: bool, whether the external validation is on high miss rate or low miss rate
        """
        ...
        self.if_high_miss = if_high_miss
        self.input_feat_pids, self.lapr_feat_pids, self.step_gt, self.max_sr,\
    ...
    def process(self, pid_data, ks=(9, 15)):
        ...
        for part_id, (conv_feat, feature_list, step_rate_mean, step_rate_std) in enumerate(tqdm(pid_data)):
            ...
            #### get the raw step rate input feature ####
            ...
            if self.if_high_miss:
                folder = f"{FILE_CACHE}/extvalid_one_layer_model_raw_feat_high_miss"
            else:
                folder = f"{FILE_CACHE}/extvalid_one_layer_model_raw_feat_low_miss"
            filename = f"pid_{pid}_test.xz"
        ...
        lapr_feat_pids = torch.cat(lapr_feat_list, dim=0).float() 
        ...
        return input_feat_pids, lapr_feat_pids, step_gt_pids, max_step_rate_pids, step_rate_mean_pids, step_rate_std_pids, part_id_pids
    ...
    def __getitem__(self, index):
        ...
        return self.input_feat_pids[index, ...], self.lapr_feat_pids[index, ...], self.step_gt[index, ...], \
...
class Batch_Collate:
    ...
    def __call__(self, batch):
        ...
        # lapr_feat_bs: shape [bs, kw, kh+ctx_len*2]. in our case is [N, 23, 153]
        input_feat_bs, lapr_feat_bs, step_gt_bs, max_sr_bs, sr_mean_bs, sr_std_bs, pid_ids_bs = zip(*batch)
        
        # stack them along a new axis inserted at dim=0
        input_feat_bs = torch.stack(input_feat_bs)
        lapr_feat_bs = torch.stack(lapr_feat_bs)
        ...
        # do the transformation for lapr_feat_bs
        lapr_feat_bs = lapr_feat_bs.unsqueeze(2) # [N, 23, 1, 153]
        kw = lapr_feat_bs.shape[1]
        lapr_feat_bs = self.unfold(lapr_feat_bs) # [N, 3335, 9]
        lapr_feat_bs = lapr_feat_bs.transpose(1, 2)  # [N, 9, 3335]
        lapr_feat_bs = lapr_feat_bs.reshape(lapr_feat_bs.shape[0], lapr_feat_bs.shape[1], kw, -1) # [N, 9, 23, 145]
        lapr_feat_bs = lapr_feat_bs.reshape(lapr_feat_bs.shape[0], -1, lapr_feat_bs.shape[3])  # [N, 9*23, 145]
        
        return input_feat_bs, lapr_feat_bs, step_gt_bs, max_sr_bs, sr_mean_bs, sr_std_bs, pid_ids_bs
